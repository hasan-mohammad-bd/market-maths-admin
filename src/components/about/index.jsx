...
const AboutList = () => {
  ...
  const fetchAboutList = async (payload) => {
    return getDataManager(about?.getAbout, setLoading, payload).then((x) => {
      console.log(x);
      if (x?.status) {
        setPagination({
          ...pagination,
  ...
  const columns = [
    ...
    {
        title: "Image",
        dataIndex: "short_about_img",
        key: "short_about_img",
        render: (_, record) => <Image width={50} src={record?.short_about_img} />,
      },
    {
      title: "About",
      dataIndex: "about",
  ...
  return (
    <TajiraCard heading="About">
      <TajiraTable
        fetchData={fetchAboutList}
        dataSource={aboutList}
        columns={columns}
        title="All About Info"
        loading={loading}
